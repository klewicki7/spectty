...
- [ ] 4.1 Create `src-tauri/src/pty_state.rs` — `PtyId = String`, `PtyState { adapter: PtyAdapter, stop: Arc<AtomicBool>, reader_thread: Option<JoinHandle<()>> }`, `#[derive(Default)] PtyRegistry(pub Mutex<HashMap<PtyId, PtyState>>)`. Registry-SHAPED; MUST NOT import a Core `SessionRegistry`. `[REQ:pty-bridge/registry-shaped-state → Scenario: keyed by PTY id]` `[unit]`
- [ ] 4.2 RED: write command-layer tests against a `FakePtyTransport` (records calls): `send_input_writes_bytes_to_transport`, `pty_resize_forwards_cols_rows`, `pty_kill_invokes_transport_kill`, `send_input_unknown_id_returns_err`. `[REQ:pty-adapter/accepts-input-resize-kill → Scenario: command-layer via fake]` `[REQ:pty-bridge/lifecycle-commands]` `[unit]`
- [ ] 4.3 GREEN: implement `src-tauri/src/commands/pty.rs` command bodies (`send_input` / `pty_resize` / `pty_kill`) operating on `&mut dyn PtyTransport` looked up by id in the registry; unknown id → `Err`. Errors map via `.map_err(|e| e.to_string())` (ping convention); mutex lock → `.lock().map_err(|_| "pty registry mutex poisoned".to_string())?` (no panic at command boundary). `[REQ:pty-bridge/lifecycle-commands]` `[REQ:pty-bridge/registry-shaped-state]` `[unit]`
- [ ] 4.4 GREEN: implement `pty_spawn` (`async`, owned types only — `cols: u16, rows: u16, cwd: Option<String>, on_output: Channel<Vec<u8>>, registry: State<'_, PtyRegistry>) -> Result<PtyId, String>`): open `PtyAdapter`, generate id, insert `PtyState`, return id. `[REQ:pty-bridge/lifecycle-commands → Scenario: pty_spawn returns id]` `[unit]/[manual]`
- [ ] 4.5 GREEN: spawn the dedicated read thread (`std::thread::Builder::new().name("pty-read-{id}")`, ADR-3 deviation from `spawn_blocking`): reused `[0u8; READ_BUF]`, `Coalescer::new(MAX_CHUNK, FLUSH_INTERVAL, Instant::now())`, `push` + `drain_due` per read → `on_output.send(chunk)`; `Ok(0)`/error → `drain_all` then break; handle `ErrorKind::Interrupted` continue. Define `READ_BUF`/`MAX_CHUNK`/`FLUSH_INTERVAL` as module consts. `[REQ:pty-adapter/streams-bytes-off-thread]` `[REQ:pty-adapter/coalesces-output]` `[unit]/[manual]`
- [ ] 4.6 GREEN: output transport = raw `Vec<u8>` over `ipc::Channel` (NO `number[]`, NO base64); `PtyExit { id, code: Option<i32> }` (`Clone, serde::Serialize`) emitted via `app.emit("pty_exit", ...)` using the v2 `Emitter` trait on thread exit. `[REQ:pty-bridge/channel-output-event-exit → Scenario: Channel vs event]` `[REQ:pty-adapter/streams-bytes-off-thread → Scenario: raw bytes]` `[unit]/[manual]`
- [ ] 4.7 GREEN: `pty_kill` lifecycle shutdown — set `stop`, `child.kill()` (closes master → read EOF), `join()` the `reader_thread`, remove the registry entry; add `Drop for PtyState` doing the same best-effort (no leaked thread). `[REQ:pty-adapter/accepts-input-resize-kill]` `[unit]/[manual]`
- [ ] 4.8 Register all four commands in `generate_handler!` and `.manage(PtyRegistry::default())` in `src-tauri/src/lib.rs`; add `pub mod pty;` to `src-tauri/src/commands/mod.rs` and `pub mod pty_state;` to `lib.rs`. `[REQ:pty-bridge/lifecycle-commands → Scenario: all four registered]` `[unit]`
- [ ] 4.9 Verify `capabilities/default.json` is UNCHANGED — `core:default` covers custom invoke + Channel; `core:event:default` covers `pty_exit` (expected delta: NONE; add a specific permission ONLY if invoke is denied at runtime in WU-7). `[REQ:pty-bridge/channel-output-event-exit]` `[ci]/[manual]`
- [ ] **Gate (WU-4)**: `cargo test --workspace` green (4 command-fake tests + all prior); `cargo fmt --check`; `cargo clippy ... -- -D warnings` (hot-loop buffer reuse, `#[must_use]` honored); `cargo deny --manifest-path crates/core/Cargo.toml check bans` exits 0.
