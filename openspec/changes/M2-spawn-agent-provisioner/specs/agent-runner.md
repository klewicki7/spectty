...
> change-level `spec.md`; this file is the promotion-ready capability delta.
...
ONLY in adapters; the Core port carries none of them.
...
`quick_actions` present as honest, tested skeletons. The trait MUST NOT expose a
`provisioner()` method (provisioning is the separate `ProvisioningPort`; this supersedes the
ADR-0004 method shape for M2).
...
`ClaudeCodeRunner::detect_status` MUST map an `OutputSignal` to an observed `AgentStatus` via
...
- **Then** it MUST return `Some(AwaitingInput)`, asserted as a pure function with no real PTY
...
- **Then** `detect_status` MUST return `Some(Completed)`, asserted deterministically
