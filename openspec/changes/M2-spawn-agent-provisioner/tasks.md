...
- [x] 6.4 GREEN: create `crates/adapters/src/agent/output_signal.rs` — `OutputSignalProducer { window: String, ansi_state: AnsiState, window_bytes, exit_code }`; `ingest(&mut self, &[u8])` (Ground/Esc/Csi/Osc/OscEsc state machine, append printable, truncate front, ANSI state persists across ingests); `mark_exit(&mut self, i32)`; `#[must_use] snapshot(&self, Timestamp, u64, bool) -> OutputSignal`. PURE — no clock, no thread, no I/O. Hand-rolled ANSI (NO new dep, D11). `[REQ:output-signal/independent-consumer]` `[unit]` `[D9][D10]`
