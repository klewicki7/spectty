...
- [x] 3.3 RED: `transition_no_op_observation_leaves_current` + the spec-faithful illegal-jump tests `transition_illegal_jump_starting_to_completed_is_rejected` (`transition(Starting, Finished)==Starting`, the spec's NAMED "illegal jump rejected" scenario) and `transition_awaiting_input_finish_is_rejected` (`transition(AwaitingInput, Finished)==AwaitingInput`), plus the legal `transition_idle_timeout_completes_from_idle` (`Idle→Completed`, exit-criterion 5) and `transition_running_to_completed_on_finish`. CORRECTION (PR1b spec-violation fix): the earlier note here was WRONG — the spec (RFC-2119 authoritative) enumerates `Completed` reachable ONLY from an active state (`Running`, + `Idle` idle-timeout), so `(Starting, Finished)` and `(AwaitingInput, Finished)` are NOT `Completed`; the prior blanket `(_, Finished) => Completed` violated the spec and masked the bug. Spec wins over the design §3.4 code block. `[REQ:agent-status-machine/pure-transition → Scenario: illegal jump rejected]` `[unit]` `[D8]`
...
- [x] 5.1 GREEN: create `crates/core/src/ports/agent_runner.rs` — `trait AgentRunner: Send + Sync` with `launch_spec(&self, &LaunchContext) -> LaunchSpec`, `detect_status(&self, &OutputSignal) -> Option<Observed>` (D8), `descriptor(&self) -> AgentDescriptor`, default `tier(&self) -> AgentTier` (= `descriptor().tier`, honors the spec "reports its tier via `tier()`" scenario), default-skeleton `parse_cost -> None` + `quick_actions -> Vec::new()`; `LaunchSpec { program, args, env: Vec<(String,String)> (sorted, D20), cwd, cols, rows }`; `LaunchContext { cwd, cols, rows, session_id, user_command }`. NO `provisioner()` method (D7/R9). NO agent-name literal. `[REQ:agent-runner/core-port-m2-subset]` `[unit]` `[D7][D8][D20]`
- [x] 5.2 GREEN: re-export `AgentRunner`/`LaunchSpec`/`LaunchContext` from core `ports/mod.rs` + `lib.rs`; `cargo deny` stays green (still serde+thiserror — `bans ok`). `[REQ:hexagonal-core/core-no-new-deps]` `[ci]`
- [x] 5.3 RED: `generic_detect_status_*` table — `(exit 0→Finished)`, `(exit≠0→Failed)`, `(is_active→Working)`, idle boundary `(idle_ms==timeout→Finished)` + `(idle_ms<timeout→Ready)` + `(idle_ms>timeout→Finished)` using injected `OutputSignal.idle_ms` (no wall clock). RED proven by inverting `>=`→`>` (boundary test FAILED `left Some(Ready)` vs `right Some(Finished)`), reverted to GREEN. `[REQ:agent-runner/generic-idle-timeout → Scenario: Completed after idle window]` `[unit]` `[D10]`
- [x] 5.4 RED: `generic_launch_spec_uses_user_command` + `generic_launch_spec_falls_back_to_default_shell` — user_command maps through; absent → per-OS default shell (reuses `config::default_shell`). Exact `LaunchSpec` equality. `[REQ:agent-runner/claude-launch-spec]`(Generic analog) `[unit]` `[D20]`
- [x] 5.5 GREEN: create `crates/adapters/src/agent/generic.rs` — `GenericRunner { idle_timeout_ms, default_program }` (default shell resolved at construction via injected `get_env`, deterministic); `detect_status` per the table; `launch_spec`; `descriptor` (tier `Generic`, `requires_provisioning: false`). `[REQ:agent-runner/generic-idle-timeout]` `[REQ:agent-runner/agent-spec-value-types → Scenario: AgentTier Generic]` `[unit]` `[D7]`
- [x] 5.6 RED: `claude_detect_status_*` — each `awaiting_input` pattern → `NeedsInput`; each `ready` pattern → `Ready`; `is_active` w/o match → `Working`; no match + inactive → `None`; exit 0 → `Finished`; exit≠0 → `Failed`; awaiting-input wins over active. `[REQ:agent-runner/claude-detect-patterns → Scenario: permission-prompt→AwaitingInput]` `[REQ:agent-runner/claude-detect-patterns → Scenario: unrecognized→None]` `[unit]` `[D11]`
- [x] 5.7 RED: `claude_launch_spec_program_cwd_and_session_env` + `claude_launch_spec_ignores_user_command` — program `claude`, `cwd==ctx.cwd`, env carries `SPECTTY_SESSION_ID`. Exact `LaunchSpec` equality. `[REQ:agent-runner/claude-launch-spec → Scenario: maps context to program/cwd/env]` `[unit]` `[D20]`
- [x] 5.8 GREEN: create `crates/adapters/src/agent/claude_code.rs` — `ClaudeCodeRunner { patterns: ClaudePatterns }` with `awaiting_input`/`ready` as `&'static [&'static str]` DATA (R5/D11, hand-rolled `contains`, NO regex); `detect_status`; `launch_spec`; `descriptor` (tier `Cooperative`, `requires_provisioning: true`, `structured_permissions: false`). `[REQ:agent-runner/claude-detect-patterns]` `[REQ:agent-runner/claude-launch-spec]` `[REQ:agent-runner/agent-spec-value-types → Scenario: AgentTier Cooperative]` `[unit]` `[D11]`
- [x] 5.9 RED+GREEN: `*_parse_cost_is_zero_skeleton` + `*_quick_actions_is_empty_skeleton` on BOTH runners — assert the honest skeleton contract (default trait methods). `[REQ:agent-runner/skeletons → Scenario: parse_cost returns zero/empty]` `[unit]`
- [x] 5.10 GREEN: create `crates/adapters/src/agent/mod.rs` — `AgentRunnerRegistry::with_builtin()` (+ `with_generic_idle_timeout`) mapping `AgentKind` string → `Box<dyn AgentRunner>` (`"claude-code"` / `"generic"`, D12); `resolve(&AgentKind) -> Option<&dyn AgentRunner>`. Re-export runners + registry from adapters `lib.rs`. `[REQ:agent-runner/core-port-m2-subset]` `[unit]` `[D12]`
- [x] **Gate (WU-5)**: `cargo test --workspace` green (core 35, adapters 40 [+28], tauri lib 10, mcp 0; 0 failed); `cargo fmt --all -- --check` clean (exit 0); `cargo clippy --workspace --all-targets -- -D warnings` exit 0; `cargo deny --manifest-path crates/core/Cargo.toml check bans` → `bans ok` (the AgentRunner trait added NO Core dep, and NO agent name leaked into Core); `cargo build -p spectty` Finished.
...
- [x] 6.1 RED→GREEN: `producer_strips_ansi_csi_sgr_sequences` + `producer_strips_csi_cursor_moves` + `producer_strips_osc_title_sequence_bel_terminated` + `producer_strips_osc_title_sequence_st_terminated` + `producer_handles_escape_split_across_two_ingests` — feed raw chunks with CSI/OSC/ESC sequences (incl. one split across two ingests) → snapshot `text_window` contains only printable text. `[REQ:output-signal/independent-consumer → Scenario: strips ANSI, bounded window]` `[unit]`
- [x] 6.2 RED→GREEN: `producer_window_truncates_from_front_at_window_bytes` — feed > `window_bytes` → window bounded, oldest dropped. RED proven by inverting the truncation (`drain(..cut)` → `truncate(keep)`): test FAILED `left "01234567"` vs `right "456789ab"`, reverted to GREEN. `[REQ:output-signal/independent-consumer → Scenario: bounded rolling window]` `[unit]`
- [x] 6.3 RED→GREEN: `producer_snapshot_carries_caller_supplied_time_fields` + `producer_mark_exit_sets_exit_code_and_preserves_window` + `producer_quiesce_then_exit_makes_ready_before_finished_reachable` — `snapshot(last_byte_at, idle_ms, is_active)` echoes the caller's clock fields into the `OutputSignal` (clock owned by caller, D10); `mark_exit(code)` sets `exit_code` WITHOUT erasing the window so a quiescent `Ready` snapshot is observable before the terminal-exit snapshot (PR2a fresh-review carry-forward). `[REQ:output-signal/non-instant-time]` `[unit]` `[D10]`
- [x] 6.4 GREEN: create `crates/adapters/src/agent/output_signal.rs` — `OutputSignalProducer { window: Vec<u8>, ansi_state: AnsiState, window_bytes, exit_code }`; `ingest(&mut self, &[u8])` (Ground/Esc/Csi/Osc/OscEsc state machine, append printable RAW BYTES, truncate front by byte cap then forward-scan to the next UTF-8 lead byte, ANSI state persists across ingests); `mark_exit(&mut self, i32)`; `#[must_use] snapshot(&self, Timestamp, u64, bool) -> OutputSignal` decoding the byte window via `String::from_utf8_lossy` so multibyte UTF-8 (e.g. the `❯` Claude prompt marker) survives byte-wise ingest. PURE — no clock, no thread, no I/O. Hand-rolled ANSI (NO new dep, D11). `[REQ:output-signal/independent-consumer]` `[unit]` `[D9][D10]`
  - PR2b fresh-review FIX: window was a `String` filled via `byte as char` (Latin-1 mapping), which corrupted any multibyte UTF-8 fed byte-by-byte and broke `text_window.contains("❯ 1. Yes")`. Now `Vec<u8>` + lossy decode at snapshot, truncation forward-scans to a UTF-8 lead byte. Added regression tests `producer_preserves_multibyte_utf8` / `..._split_across_ingests` / `producer_truncate_window_on_multibyte_boundary`, plus contract-pinning `producer_lone_esc_then_printable` / `producer_csi_private_and_intermediate_bytes` / `producer_preserves_newline_tab_drops_other_control`.
- [x] 6.5 GREEN: re-export `OutputSignalProducer` from adapters `lib.rs` + `agent/mod.rs`. `[REQ:output-signal/independent-consumer]` `[ci]`
- [x] **Gate (WU-6)**: `cargo test --workspace` green (adapters 40→49, +9 producer tests; core 35, tauri lib 10, mcp 0; 0 failed); `cargo fmt --all -- --check` clean (exit 0); `cargo clippy --workspace --all-targets -- -D warnings` exit 0; `cargo deny --manifest-path crates/core/Cargo.toml check bans` → `bans ok`; `cargo build -p spectty` Finished.
...
**Strict TDD**: RED the foreign-key VALUE+ORDER preservation property FIRST (R7 headline), then scope, then file-IO seam, then the adapter.
**R7 contract (corrected after PR3 adversarial review)**: inject/retract preserve every foreign key's VALUE and — with `serde_json`'s `preserve_order` feature enabled for the ADAPTERS crate ONLY (backs `Value` with `indexmap`; stays OUT of Core's `-e normal` graph, `cargo deny` bans still green) — its relative ORDER. Whitespace/indentation is NORMALIZED by the pretty serializer; the result is NOT byte-identical to arbitrary hand-formatted input (`~/.claude.json` is machine-managed JSON, so VALUE+ORDER preservation is the contract, not text preservation). The original "byte-identical" claim was FALSE for hand-formatted input and its headline test was tautological (fixture pre-built via `to_string_pretty`).
...
- [x] 7.1 GREEN: create `crates/core/src/ports/provisioning.rs` — `trait ProvisioningPort: Send + Sync { inject(&self, ProvisioningScope) -> Result<ProvisioningHandle, ProvisioningError>; retract(&self, &ProvisioningHandle) -> Result<(), ProvisioningError>; }`; `enum ProvisioningScope { Global, Project(String) }`; `struct ProvisioningHandle { scope }`; `enum ProvisioningError { Io(String), Parse(String) }` (thiserror). SEPARATE from AgentRunner (Lock 1/D7). NO refresh() (M3). NO agent name / config path in Core. Re-export from `lib.rs`. `[REQ:provisioning-port/core-trait-separate → Scenario: inject+retract, distinct trait]` `[unit]` `[D7]`
- [x] 7.2 RED: `inject_then_retract_preserves_hand_formatted_foreign_values` — HAND-FORMATTED config (non-alphabetical key order, 4-space indent, inline objects; `theme`/`numStartups`/`projects` + `mcpServers` with `user-tool`/`gentle-ai`): inject spectty → retract → every foreign VALUE preserved AND (with `preserve_order`) relative key ORDER preserved (asserted against RAW serialized text, not re-parsed Values), no `spectty` left behind. Plus `inject_then_retract_is_stable_on_already_canonical_input` (round-trip STABILITY of canonical input only — not arbitrary-input preservation) and `non_object_mcp_servers_is_a_parse_error_not_data_loss` (pre-existing `"mcpServers": []` → `ProvisioningError::Parse`, never clobbered). NOTE (PR3 review fix): the old `inject_then_retract_round_trips_foreign_keys_byte_identical` test was tautological (fixture pre-normalized via `to_string_pretty`) and the byte-identical doc claim was FALSE for hand-formatted input — both corrected. `[REQ:provisioning-port/json-namespace → Scenario: inject leaves foreign keys untouched]` `[unit]` `[D17]`
- [x] 7.3 RED: `retract_removes_only_spectty_keys` + `inject_is_idempotent` (double inject == single) + `inject_on_missing_mcpServers_creates_valid_json`. `[REQ:provisioning-port/json-namespace → Scenario: retract removes only spectty_*]` `[REQ:provisioning-port/json-namespace → Scenario: malformed/missing mcpServers handled]` `[unit]` `[D17]`
- [x] 7.4 GREEN: create `crates/adapters/src/provision/json_namespace.rs` — PURE `inject_spectty_mcp(current_json, server_name, &McpServerEntry) -> Result<String, ProvisioningError>` + `retract_spectty_mcp(current_json, server_name) -> Result<String, ProvisioningError>` over `serde_json::Value`, mutate ONLY the `spectty`-prefixed sub-keys under `mcpServers`, re-serialize pretty. `McpServerEntry { command, args, env }`. NO text markers, NO `claude mcp add`. `[REQ:provisioning-port/json-namespace]` `[unit]` `[D17]`
- [x] 7.5 RED: `resolve_scope_table` — fake `is_git_tracked`: tracked → `Project(root)` (`.mcp.json`); untracked/absent → `Global` (`~/.claude.json`). `[REQ:provisioning-port/scope-global-default-project-when-tracked → Scenario: git-tracked→PROJECT]` `[REQ:provisioning-port/scope-global-default-project-when-tracked → Scenario: untracked→GLOBAL]` `[unit]` `[D18]`
- [x] 7.6 GREEN: create `crates/adapters/src/provision/scope.rs` — `resolve_scope(repo_root, config_path, is_git_tracked: impl Fn(&str)->bool) -> ProvisioningScope` (pure); plus the real `is_git_tracked` probe (`git ls-files --error-unmatch`, exit-0→true) kept SEPARATE from the pure resolver. `[REQ:provisioning-port/scope-global-default-project-when-tracked]` `[unit]` `[D18]`
- [x] 7.7 RED: `first_write_creates_spectty_bak_of_the_original` + `second_write_does_not_overwrite_the_backup` (+ `read_absent_file_is_none`) against an in-memory `FakeConfigFile`. The crash-mid-write/`interrupted_write_leaves_original_intact` scenario is satisfied STRUCTURALLY by the atomic-write seam contract (temp→fsync→rename in `RealConfigFile`: the original is only ever replaced by an os-atomic `rename`, never partially written) + the backup is taken before the first write; the real-FS interruption path is a `[manual]`/integration concern (WU-11/12), not a unit fake assertion. `[REQ:provisioning-port/atomic-write-backup → Scenario: first write creates .spectty.bak]` `[REQ:provisioning-port/atomic-write-backup → Scenario: crash mid-write never partial]` `[unit]`
- [x] 7.8 GREEN: create `crates/adapters/src/provision/file_io.rs` — `trait ConfigFile: Send + Sync { read(&self,&str)->io::Result<Option<String>>; write_atomic(&self,&str,&str)->io::Result<()>; }`; `RealConfigFile` (tmp→fsync→rename, `.spectty.bak` on first write); `FakeConfigFile` (HashMap, test-only). `[REQ:provisioning-port/atomic-write-backup]` `[unit]`
- [x] 7.9 RED: `claude_provisioner_inject_writes_scope_path_and_backs_up` + `retract_absent_file_is_ok` against `FakeConfigFile`. `[REQ:provisioning-port/inject-on-create-retract-on-close]` `[unit]`
- [x] 7.10 GREEN: create `crates/adapters/src/provision/claude_provisioner.rs` — `ClaudeJsonProvisioner<F: ConfigFile> { files, home_claude_json, mcp_entry }` impl `ProvisioningPort`: `inject` resolves path per scope, read-or-default, `inject_spectty_mcp`, `write_atomic`, return handle; `retract` read→`retract_spectty_mcp`→write (absent = `Ok(())`). `mcp_entry.command` points at the spectty-mcp binary. Re-export from adapters `lib.rs` + `provision/mod.rs`. `[REQ:provisioning-port/inject-on-create-retract-on-close]` `[unit]` `[D14]`
- [x] **Gate (WU-7)**: `cargo test --workspace` green (round-trip/scope/file-IO/adapter); fmt/clippy clean; `cargo deny --manifest-path crates/core/Cargo.toml check bans` exits 0 (the ProvisioningPort trait added NO Core dep).
...
- [x] 8.1 RED: `tools_list_advertises_exactly_five_schemas` — handle a `tools/list` request → response names exactly `spectty_spec, spectty_diff, spectty_approval, spectty_status, spectty_cost` with their declared input schemas (per agent-protocol.md). `[REQ:provisioning-port/spectty-mcp-stub → Scenario: advertises exactly five tool schemas]` `[unit]` `[D15]`
- [x] 8.2 RED: `tools_call_known_returns_ack_no_effect` + `tools_call_unknown_returns_-32601` + `bad_params_returns_-32602`. `[REQ:provisioning-port/spectty-mcp-stub → Scenario: stub call returns ack, no side effect]` `[unit]` `[D15]`
- [x] 8.3 RED: `initialize_returns_protocol_and_serverinfo` — `initialize` → `{ protocolVersion, capabilities:{tools:{}}, serverInfo:{name:"spectty-mcp",version} }`. `[REQ:provisioning-port/spectty-mcp-stub]` `[unit]`
- [x] 8.4 GREEN: implement `crates/spectty-mcp/src/main.rs` — pure request-dispatch fns (`handle_initialize`/`handle_tools_list`/`handle_tools_call`) testable WITHOUT spawning the process, wrapped by a stdio JSON-RPC 2.0 read loop. Known tool → `result { content:[text ack], isError:false }`; unknown → `-32601`; bad params → `-32602`. Schemas FROZEN as the M3-swap contract (R4/D15). serde/serde_json only — NO core, NO tauri (D16). Implemented as a single pure `handle_message(&str) -> Option<String>` (None for notifications) so dispatch is unit-testable without stdio; framing pinned to newline-delimited JSON-RPC (design open-question c resolved). `[REQ:provisioning-port/spectty-mcp-stub]` `[unit]` `[D15][D16]`
- [x] **Gate (WU-8)**: `cargo test --workspace` green (handler unit tests); fmt/clippy clean; `cargo build -p spectty-mcp` produces the binary. NOTE: stdio framing pinned to **line-delimited JSON-RPC 2.0** (one JSON object per line) — the official MCP stdio transport; NOT Content-Length headers. Pin re-validated against a real `claude` launch in WU-12.
...
- [x] 9.1 GROW `src-tauri/src/pty_state.rs` — `pub type PtyId = String` now sourced from `SessionRegistry::mint_id()`; RETIRE `next_pty_id` (D13). `PtyRegistry` keeps OS handles only; keyed by the SAME string as `SessionRegistry` (lockstep, no cross-map). `[REQ:session-registry/distinct-from-ptyregistry]` `[unit]` `[D13]` — _PR5a: `next_pty_id` removed from `commands/pty.rs`; `pty_spawn` mints via `sessions.mint_id().0`; `SessionRegistry` managed in `lib.rs`. M1 PTY tests stay green._
- [x] 9.2 RED: `observe_and_diff_emits_only_on_change` — fake `AgentRunner` returns observations; real `SessionRegistry`; assert `Some(new)` only when status changes, `None` on no-op / `detect_status` `None`. `[REQ:agent-session-ui/status-event → Scenario: status_changed only on actual change]` `[unit]` `[D8][D19]` — _PR5a: `ScriptedRunner` (scripted `Option<Observed>`) + real `SessionRegistry`; RED proven against a `None`-returning stub._
- [x] 9.3 GREEN: create `src-tauri/src/session_runtime.rs` — pure `observe_and_diff(runner, sessions, id, signal) -> Option<AgentStatus>` (= `runner.detect_status(signal).and_then(|o| sessions.apply_observed(id, o))`). `[REQ:agent-session-ui/status-event]` `[unit]` `[D8]` — _PR5a._
- [x] 9.4 RED: `bounded_signal_channel_drops_oldest_never_blocks` — fill a bounded `sync_channel` (cap N), assert the read-thread `try_send` side never blocks and drops on overflow (the R6 render-protection seam). `[REQ:output-signal/independent-consumer → Scenario: cannot back-pressure render]` `[unit]` `[D9]` — _PR5a: `signal_channel`/`signal_try_send` (`try_send`+drop). RED proven: a blocking `send` stub HANGS the test._
- [x] 9.5 GREEN: wire the signal thread in `session_runtime.rs` — tee a SECOND bounded `sync_channel` off the M1 read thread (`try_send`+drop, the render `tx` keeps its M1 unbounded behavior), THIRD thread runs `producer.ingest` → `clock.now()` stamp → `producer.snapshot` → `observe_and_diff` → emit `status_changed`; `recv_timeout(QUIESCE)` so idle ticks fire while quiescent (the M1 R3 insight); EOF builds a final signal with `exit_code` set so the terminal status is emitted. `[REQ:output-signal/independent-consumer]` `[REQ:agent-session-ui/status-event]` `[unit]/[manual]` `[D9][D10]` — _PR5a: `run_signal_loop` with `signal_step` (Ingest/Quiesce/Eof) + an INJECTED `emit: FnMut(StatusChanged)` seam (Tauri-free; PR5b wires `app.emit`). EOF emits a quiescent snapshot BEFORE the terminal one so `Starting→Idle→Completed` is reachable (table forbids `Starting→Completed`). `SystemClock` created in `crates/adapters/src/clock.rs`._
- [x] 9.6 RED: `spawn_session_impl_mints_inserts_and_injects_only_when_required` — fake transport + fake provisioner (records inject/retract) + fake runner registry: assert id minted, `inject` called ONLY when `requires_provisioning` (Claude yes, Generic no), registry insert, NO real PTY. `[REQ:provisioning-port/inject-on-create-retract-on-close]` `[REQ:agent-runner/agent-spec-value-types]` `[unit]` `[D7]`
- [x] 9.7 RED: `close_session_impl_kills_pty_then_retracts_and_removes` — assert PTY kill (M1 path) THEN `retract` for the session's stored scope THEN registry remove. `[REQ:provisioning-port/inject-on-create-retract-on-close → Scenario: close retracts after killing PTY]` `[unit]`
- [x] 9.8 GREEN: create `src-tauri/src/commands/session.rs` — `spawn_session` (async, owned types: `agent: AgentSpec, workspace_path, title, cols, rows, on_output: Channel<Vec<u8>>` + `State` for SessionRegistry/PtyRegistry/AgentRunnerRegistry/`Arc<dyn ProvisioningPort>`/`Arc<dyn ClockPort>`) following design §6.1 steps 1–10; `close_session`; `list_sessions`; `get_session`. Free `*_impl` fns hold the testable logic. Errors → `String` (M0/M1 convention). `[REQ:agent-session-ui/bridge-commands]` `[unit]` `[D7][D13]`
- [x] 9.9 GREEN: `StatusChanged { session_id, status, quick_actions }` (`Clone, Serialize`) emitted via v2 `Emitter` ONLY on a real transition change; also `session_created(SessionSummary)` + `session_closed(SessionId)`; `pty_exit` kept from M1. `[REQ:agent-session-ui/status-event → Scenario: payload carries session_id/status/quick_actions]` `[unit]/[manual]`
- [x] 9.10 GREEN: register the 4 new commands in `generate_handler!` and `.manage(SessionRegistry::default())` + `.manage(AgentRunnerRegistry::with_builtin())` + `.manage::<Arc<dyn ProvisioningPort>>(...)` + `.manage::<Arc<dyn ClockPort>>(SystemClock)` in `lib.rs`; `pub mod session;` in `commands/mod.rs`; `pub mod session_runtime;`. Create `crates/adapters/src/clock.rs` `SystemClock` (impl `ClockPort`) here if not already. `[REQ:agent-session-ui/bridge-commands → Scenario: spawn and close registered]` `[unit]` `[D10]`
- [x] 9.11 Verify `capabilities/default.json` is UNCHANGED (custom commands + Channel + events ride `core:default`/`core:event:default`, same as M1). `[REQ:agent-session-ui/bridge-commands]` `[ci]/[manual]`
- [x] **Gate (WU-9)**: `cargo test --workspace` green (impl fakes + observe_and_diff + bounded-channel); fmt/clippy clean; `cargo deny --manifest-path crates/core/Cargo.toml check bans` exits 0; `cargo build -p spectty` succeeds.
...
- [x] 10.1 GREEN: created `ui/src/session/ipc.ts` — typed wrappers `spawnSession(agent, workspacePath, title, cols, rows, onOutput)`, `closeSession(id)`, `listSessions()`, `getSession(id)` + `listenStatusChanged`/`listenSessionCreated`/`listenSessionClosed` helpers and `AgentSpec`/`AgentStatus`/`SessionSummary`/`StatusChangedPayload` types (camelCase JS ↔ snake_case Rust args via Tauri v2 default; snake_case event payloads `session_id`/`quick_actions`/`agent_kind`). `[REQ:agent-session-ui/ui-spawn-and-status]` `[unit]`
- [x] 10.2 RED→GREEN: `useSession.test.ts` — `spawn() invokes spawn_session with the chosen agent and workspace`. `[REQ:agent-session-ui/ui-spawn-and-status → Scenario: selecting agent+workspace invokes spawn]` `[unit]`
- [x] 10.3 RED→GREEN: `status_changed_updates_badge_and_shows_title` — fire a mocked `status_changed` filtered by `session_id` → `useSession` status updates, ignores other sessions; `PaneHeader.test.tsx` exhaustively maps the 6 statuses → label/variant and renders title. `[REQ:agent-session-ui/ui-spawn-and-status → Scenario: Pane header badge updates on status_changed]` `[unit]`
- [x] 10.4 RED→GREEN: `close() invokes close_session with the current session id` + `session_closed clears the session`. `[REQ:agent-session-ui/ui-spawn-and-status]` `[unit]`
- [x] 10.5 GREEN: created `ui/src/hooks/useSession.ts` — spawn/close orchestration + `status_changed`/`session_closed` listeners (mirrors `useTerminal`: one `useEffect` owns the listener lifecycle; `sessionIdRef` filters events to the current session without re-subscribing); React 19 named imports; NO manual `useMemo`/`useCallback`. `[REQ:agent-session-ui/ui-spawn-and-status]` `[unit]/[manual]`
- [x] 10.6 GREEN: created `ui/src/components/SpawnDialog.tsx` — agent radio (Claude Code | Generic + free-text command parsed to `string[]` only for Generic), workspace text field (avoids a dialog plugin/permission in M2), title field → `onSpawn` → `spawnSession`. `[REQ:agent-session-ui/ui-spawn-and-status]` `[unit]/[manual]`
- [x] 10.7 GREEN: created `ui/src/components/PaneHeader.tsx` — `title` + `AgentStatus` badge via pure `statusBadge()` (Starting=grey, Idle=blue, Running=green-pulse, AwaitingInput=amber-pulse, Completed=grey-check, Error=red; `data-status` keys the CSS modifier). Status flows DOWN from `useSession` (App owns the subscription) so the UI NEVER computes status locally (backend authoritative). Wired `<PaneHeader/>`+`<SpawnDialog/>` around the M1 `<Terminal/>` in `App.tsx`. `[REQ:agent-session-ui/ui-spawn-and-status]` `[unit]/[manual]`
- [x] **Gate (WU-10)**: `pnpm -C ui test` green (38 tests, 27 new: session-ipc 8 + useSession 7 + PaneHeader 11; 11 M1 still pass); `pnpm -C ui build` typechecks (TS strict) and bundles; `cargo build -p spectty` green (Rust untouched).
