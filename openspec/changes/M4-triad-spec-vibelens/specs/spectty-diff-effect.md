...
signal that triggers the diff pipeline immediately, bypassing the FileWatch debounce. The
`emits_diff_signals` capability (false for claude-code today) governs the cooperative path; otherwise
the FileWatch fallback drives the pipeline.
...
the FileWatch debounce, then return promptly. With no cooperative signal (generic tier), the FileWatch
debounced trigger MUST drive the pipeline. Both paths converge on the SAME pipeline.
