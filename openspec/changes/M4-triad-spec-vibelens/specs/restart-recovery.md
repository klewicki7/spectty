...
Restarting Spectty mid-session MUST restore the spec + progress from engram (roadmap exit criterion 6)
because engram IS the store (Decision 1).

## ADDED Requirements

### Requirement: Spec and progress are restored from engram on restart
On session re-attach after restart, Spectty MUST read `spectty/{session_id}/spec` (and `/progress`)
from engram and reconstruct the `SpecContract` so the Spec pane shows the prior intent, plan, task
states, and approval. If engram is unreachable at restart, the pane MUST degrade gracefully
(empty/last-known) without crashing.
