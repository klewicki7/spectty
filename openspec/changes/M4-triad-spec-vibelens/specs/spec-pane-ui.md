...
The UI gains a Spec pane: seeds intent, renders the living checklist reacting to `spec_updated` (no
manual refresh), and presents the plan-approval gate (Approve/Edit/Reject) calling `approve_prompt`.
Follows the M1/M2 hook pattern (`useSession`-style, React 19 named imports, no manual
`useMemo`/`useCallback`); `invoke`/listeners mocked in vitest. Generic tier shows a coarse scraped badge.
...
When `approval == Pending`, the Spec pane MUST present Approve, Edit, and Reject actions; selecting one
MUST invoke `approve_prompt(session_id, action_id, decision)`. The gate MUST NOT render once approval
is resolved.
