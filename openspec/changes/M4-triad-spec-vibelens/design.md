...
**Amendment (PR-5, reconciled with adversarial review F1).** The D37/D34 sketch placed the dedup state on the `Session` aggregate (`Session.last_diff` / `last_diff_hash`, mutated via `Session::update_diff` → emit `diff_updated`). The shipped pipeline instead keeps the dedup state (current explanation + hash) on the per-session `DiffPipeline` in the bridge (`src-tauri/src/diff_pipeline.rs`, `PipelineState`). Rationale: the pipeline is the SOLE owner of a session's diff cadence, so co-locating the hash there is behaviourally identical AND keeps Core untouched this slice (R6 quarantine forbids new Core code in PR-5). Consequences verified by review: restart-recovery does NOT require diff state (diff is transient enrichment per D38, only spec+progress hydrate), and `get_diff_explanation` reads the pipeline cache — so nothing is lost by not writing through `Session`. The PR-4 Core fields `Session.last_diff` / `last_diff_hash` + `Session::update_diff` remain as Core capacity for the future session-registry diff surface (M5 dashboard wiring). The delta specs (`specs/spec.md` M4-REQ-13, `specs/diff-pipeline.md`) were reconciled to describe the pipeline-owned dedup state.
